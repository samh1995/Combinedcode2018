...
%        else if SimParams.useDandreaRecovery == 1 %% we can declare early on if ew wnanna use dandrea controller or not
%           if Control.shutdown==1 %this is true when impacts occur and the prop is lost, trigger this and dand concotlller activates
%             [Control] = controllerrecovery(tStep, Pose, Twist, Control);    %whole  
%             Control.type = 'DandActicated';
%           else 
%               [Control] = checkrecoverystage(Pose, Twist, Control, ImpactInfo);
%             [Control] = computedesiredacceleration(Control, Twist); 
% 
